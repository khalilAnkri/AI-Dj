...
services.py — complete file with:
  ...
  - PredictionService     : Vertex AI endpoint inference with confidence + top features
  - ExplanationService    : rule-based natural language explanation of prediction
  - RecommendationService : Musicae RapidAPI similar track recommendations
...
class SpotifyService:
    ...
    - get_metadata() : track name, artist, thumbnail via Musicae /tracks (oEmbed fallback)
    """
    ...
    def get_metadata(self, track_id: str) -> dict:
        ...
        Falls back to Spotify oEmbed if Musicae fails.
        Returns empty dict on total failure — metadata is non-critical.
        """
        ...
        try:
            ...
            print(
                f"Musicae /tracks returned {response.status_code}, falling back to oEmbed."
            )
    ...
    def _fetch_from_musicae(self, track_id: str, retries: int = 3) -> dict:
        """
        Fetches audio features from Musicae with exponential-backoff retry
        on 429 responses to handle Musicae's undocumented burst-rate limit.
        """
        ...
        for attempt in range(retries):
            ...
            if response.status_code == 429:
                ...
                print(
                    f"Musicae 429 rate-limit hit — retrying in {wait}s "
            ...
            if response.status_code == 401:
                raise Exception(
                    "Musicae API: 401 Unauthorized — check your RAPIDAPI_KEY."
                )
        ...
        raise Exception(
            f"Musicae API: 429 Rate limit exceeded after {retries} retries. "
