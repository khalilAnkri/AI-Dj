...
def call_predict(query: str) -> dict:
    try:
        r = requests.post(f"{API_BASE}/predict", json={"query": query}, timeout=15)
        r.raise_for_status()
        return r.json()
...
def call_predict_manual(features: dict) -> dict:
    try:
        r = requests.post(f"{API_BASE}/predict_manual", json=features, timeout=15)
        r.raise_for_status()
        return r.json()
    except requests.exceptions.ConnectionError:
...
def call_history() -> list:
    try:
        r = requests.get(f"{API_BASE}/past_predictions", timeout=5)
