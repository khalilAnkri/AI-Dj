...
from api import call_history, call_predict, call_predict_manual
from ui import (
    render_footer,
    render_header,
...
# ─────────────────────────────────────────────────────────────────────────────
    ...
    if active_tab == "ANALYZE":
        st.markdown(
            "<div style=\"color:#4a7060;font-family:'JetBrains Mono',monospace;"
        ...
        st.markdown(
            "<div style=\"color:#4a7060;font-family:'JetBrains Mono',monospace;"
            "font-size:9px;letter-spacing:3px;text-transform:uppercase;"
            'margin-bottom:16px;">Past Predictions</div>',
            unsafe_allow_html=True,
        )
        ...
        if not history:
            st.markdown(
                '<div style="color:#4a7060;text-align:center;padding:60px 0;'
            ...
            for item in reversed(history):
                ...
                st.markdown(
                    f"""<div style="background:#0e1e1a;border:1px solid #1e3530;
