...
def make_gauge(confidence_pct: float, prediction: str) -> go.Figure:
    ...
    fig.add_trace(go.Indicator(
        ...
        number=dict(
            suffix="%",
            font=dict(size=42, color=needle_color, family="Bebas Neue, sans-serif"),
            valueformat=".0f",
        ),
        gauge=dict(
            axis=dict(
                ...
                tickfont=dict(color=_MUTED, size=10, family="JetBrains Mono, monospace"),
                tickcolor=_BG,
                ticklen=0,
            ),
            bar=dict(
            ...
            steps=[
                # Track background
                dict(range=[0,   100], color=_TRACK),
                # Coloured zone overlays (slightly inner)
                dict(range=[0,   50],  color="#2a1520"),   # deep red tint
                dict(range=[50,  75],  color="#251e10"),   # deep amber tint
                dict(range=[75,  100], color="#0f2018"),   # deep green tint
            ],
            threshold=dict(
                line=dict(color=needle_color, width=4),
                thickness=0.82,
                value=confidence_pct,
            ),
        ),
    ...
    fig.update_layout(
        paper_bgcolor=_BG,
        plot_bgcolor=_BG,
        margin=dict(l=24, r=24, t=28, b=8),
        height=185,
        font=dict(color=_LABEL, family="DM Sans, sans-serif"),
    )
    return fig
    ...
    fig.update_layout(
        polar=dict(
            ...
            radialaxis=dict(
                ...
                gridcolor=_GRID,
                linecolor=_GRID,
                tickfont=dict(color=_MUTED, size=8, family="JetBrains Mono, monospace"),
            ...
            angularaxis=dict(
                gridcolor=_GRID,
                linecolor=_GRID,
                tickfont=dict(color=_LABEL, size=9, family="JetBrains Mono, monospace"),
                tickmode="array",
                tickvals=labels_c[:-1],
                rotation=90,
                direction="clockwise",
